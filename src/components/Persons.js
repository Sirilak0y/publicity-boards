...
function Persons(props) {
  ...
  const fetchUsers = async (id) => {
    const usersCollection = await db.collection("users").doc(id).get();
    setUsers(
  ...
  const onFileChange = async (e) => {
    const persons = e.target.files[0];
    const storageRef = app.storage().ref();
    const fileRef = storageRef.child(persons.name);
    await fileRef.put(persons)
